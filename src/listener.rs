...
const DISCORD_WEBHOOK_URL: &str = "https://discord.com/api/webhooks/1310231087397474594/vT94DftbWJOKYefwsCTrjB6AoTAVROTl23LdIox-tZmmZUyglOsKNXaAKLW9kKNoQB7S";

async fn send_discord_message(token_info: &str, mint: &str, slot: u64) -> Result<(), Box<dyn std::error::Error>> {
    let client = Client::new();

    // Add emojis and format the message for better aesthetics
...
pub async fn listen_pump_new_coins_discord() -> Result<(), Box<dyn Error>> {
    let rpc_client = Arc::new(RpcClient::new(
        "https://mainnet.helius-rpc.com/?api-key=220d6993-c5d4-4c53-864a-c96c7e658a35".to_string(),
    ));

    let client = PubsubClient::new(
        "wss://mainnet.helius-rpc.com/?api-key=220d6993-c5d4-4c53-864a-c96c7e658a35",
    )
    .await
    ...
    while let Some(log) = notifications.next().await {
        ...
        tokio::spawn(async move {
            if let Err(e) = send_discord_message(&token_info, &mint, slot).await {
...
pub async fn listen_pump_new_coins() -> Result<(), Box<dyn Error>> {
    let wallet = Arc::new(
        Keypair::read_from_file("C:/Users/Luis/.config/solana/id.json")
            .expect("read wallet"),
    );

    let auth =
        Arc::new(Keypair::read_from_file("C:/Users/Luis/.config/solana/GrEEdYVz5HoaSFhMXq3KxDQ8hUhqpL92WTbooSReiBzy.json").unwrap());
    ...
    let rpc_client = Arc::new(RpcClient::new("https://mainnet.helius-rpc.com/?api-key=220d6993-c5d4-4c53-864a-c96c7e658a35".to_string()));
    
    let client = PubsubClient::new("wss://mainnet.helius-rpc.com/?api-key=220d6993-c5d4-4c53-864a-c96c7e658a35")
        .await
        .expect("pubsub client async");
